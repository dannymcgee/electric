...
import { NODE_FACTORY } from "./transformer"
...
describe("NODE_FACTORY", () => {
	it("includes all methods declared by its type", () => {
		const nodeFactoryTypeMembers = resolveNodeFactoryTypeMembers()
		const nodeFactoryMembers = Object.keys(NODE_FACTORY)
		...
		for (let key of nodeFactoryTypeMembers) {
			...
			expect(typeof (NODE_FACTORY as any)[key]).toBe("function")
		}
	})
})
